...
```{r}
d <-  read_csv('../data/fotohandeldelfshaven_2023-11-09.csv')
```
...
d %>% 
mutate(str_replace = str_replace(new_price, ",00", ""),
  text_column = as.numeric(str_replace(str_replace, "€", ""))) %>% 
...
all_recent_data_sold_info <- all_recent_data %>%
  mutate(
    ...
    date_sold = if_else(
      (new_price != prices) & (date_sold!='not_sold_sold_before'),
